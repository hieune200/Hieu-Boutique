            ...
            <div className="checkout-grid" style={{display:'grid', gridTemplateColumns:'1fr 1fr 420px', gap:24}}>
                <section className="checkout-left">
                    ...
                    <form className="custommer-form" onSubmit={handleSubmit}>
                        ...
                        <input required type="tel" name="tel" id="tel" placeholder='Số điện thoại' className='col12' pattern='[0-9]{10}' defaultValue={inforCheckout?.phoneNumber} onChange={(e) => handleSetInforCheckout('phoneNumber', e.target.value)}/>
                        <input required type="email" name="email" id="email" placeholder='Email' className='col12' defaultValue={inforCheckout?.email} onChange={(e) => handleSetInforCheckout('email', e.target.value)}/>
                        <input required type="text" name="address" id="address" placeholder='Địa chỉ' className='col12' defaultValue={inforCheckout?.address} onChange={(e) => handleSetInforCheckout('address', e.target.value)}/>
                        ...
                        <textarea placeholder='Ghi chú thêm (Ví dụ: Giao hàng giờ hành chính)' className='col12' onChange={(e)=>handleSetInforCheckout('note', e.target.value)} />
                    </form>
                </section>
                ...
                <section className="checkout-center">
                    ...
                    <div style={{display:'flex', flexDirection:'column', gap:12}}>
                        <button className={`pay-option ${payMethod.method === 'offline' ? 'active' : ''}`} onClick={()=>handleSetPayMethod('offline')} style={{padding:18, borderRadius:8, border: payMethod.method === 'offline' ? '1px solid #d32f2f' : '1px solid #eee', background: payMethod.method === 'offline' ? '#fff' : '#fafafa'}}>
                            <div style={{display:'flex', alignItems:'center', gap:12}}>
                                <img src={logoTp} alt="cod" style={{width:36}} />
                                <div>COD, Thanh toán khi nhận hàng</div>
                            </div>
                        </button>
                        <button className={`pay-option ${payMethod.method === 'online' ? 'active' : ''}`} onClick={()=>handleSetPayMethod('online')} style={{padding:18, borderRadius:8, border: payMethod.method === 'online' ? '1px solid #d32f2f' : '1px solid #eee', background: payMethod.method === 'online' ? '#fff' : '#fafafa'}}>
                            <div style={{display:'flex', alignItems:'center', gap:12}}>
                ...
                <aside className="checkout-right">
                    <div style={{marginBottom:12, display:'flex', gap:12}}>
                        ...
                        <button onClick={applyCoupon} className='btn' style={{background:'#c62828', color:'#fff', padding:'10px 16px', borderRadius:8}}>Áp dụng</button>
                    </div>
                    ...
                    <div className='cart-box' style={{border:'1px solid #eee', borderRadius:10, padding:12}}>
                                ...
                                <div key={idx} style={{display:'flex', gap:12, alignItems:'center'}}>
                                    ...
                                    <div style={{flex:1}}>
                                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                                            ...
                                            <button onClick={()=>handlRemoveItem(idx)} style={{border:0, background:'transparent', color:'#c62828'}}>✕</button>
                                        </div>
                                        ...
                                        <div style={{marginTop:8, display:'flex', alignItems:'center', gap:8}}>
                                            <div style={{display:'flex', alignItems:'center', border:'1px solid #222', borderRadius:8, padding:'6px 8px'}}>
                                                <button onClick={()=>changeItemQuantity(idx, -1)} style={{border:0, background:'transparent', fontSize:18}}>−</button>
                        ...
                        <div style={{marginTop:12, borderTop:'1px solid #eee', paddingTop:12}}>
                            ...
                            <div style={{display:'flex', justifyContent:'space-between'}}><div>Giảm giá</div><div>{formatCurrency(discountAmount)}</div></div>
                            <div style={{display:'flex', justifyContent:'space-between'}}><div>Phí giao hàng</div><div>{formatCurrency(25000)}</div></div>
                            <hr />
                            <div style={{display:'flex', justifyContent:'space-between', fontSize:20, fontWeight:700}}><div>Tổng</div><div>{formatCurrency(Math.max(0, totalBill - discountAmount) + 25000)}</div></div>
                        </div>

                        <div style={{marginTop:12, display:'flex', gap:8}}>
                            <button className='btn' onClick={()=>nav('/')} style={{flex:1, background:'#eee'}}>Trở về trang chủ</button>
                            <button className='btn' onClick={(e)=>handleSubmit(e)} style={{flex:1, background:'#c62828', color:'#fff'}}>Xác nhận đặt hàng</button>
                        </div>
                    </div>
                </aside>
            </div>
